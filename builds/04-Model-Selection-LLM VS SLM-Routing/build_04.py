...
# =============================================================================
# HELPER FUNCTIONS (from Builds 1, 2, 3)
# =============================================================================
...
# =============================================================================
# INTENT CLASSIFIER & MODEL ROUTING (NEW in build 4)
# =============================================================================
...
Classify the user's message as either "simple" or "complex".

Classify as "simple" when the message is:
- A greeting
- An identity question
- A short customer support question
- A basic factual question
- A simple status check
- A short transactional request

Examples of simple:
- "Hi"
- "Hello"
- "Are you the AI agent for this company?"
- "Can you help me?"
- "What is my refund status?"
- "Where is my order?"

Classify as "complex" only when the message requires:
- Analysis
- Comparison
- Evaluation
- Planning
...
- "Compare these two products and recommend the best one."
- "Create a plan to reduce customer complaints."
- "Evaluate the risks of this approach."

If the message is unclear or borderline, classify it as "complex".
...
def route_to_model(user_question, messages, use_slm_classifier=True):
    ...
    Uses deterministic rules first, then SLM classification if needed.
    Returns: (response_text, model_name, latency_ms, token_usage)
    """

    rule_based_intent = classify_intent(user_question)

    if rule_based_intent == "simple":
        print("\n[INTENT CLASSIFIER] Rule-based classifier detected SIMPLE request.")
        intent = "simple"
    elif use_slm_classifier:
...
# Get user input
#user_message_text = "Hi are you the ai agent for this comanny"
#user_message_text = "My refund request has been pending for three weeks. Can you analyze the possible reasons for the delay, explain what might be happening internally, and recommend the best next steps I should take?"
